...
class GraphModelForMultiTask(MultiTaskForward):
    ...
    def __init__(
        ...
        self.node_encoder = (
            None
            if node_encoder_config == None
            else node_encoder_config.get_model()
        )
        self.edge_encoder = (
            None
            if edge_encoder_config == None
            else edge_encoder_config.get_model()
        )
        ...
        if self.node_encoder != None:
            if isinstance(self.node_encoder, WordEncoder):
                data.x = self.node_encoder(
                    data.x, getattr(data, "extra_x", None)
                )
        ...
        if self.edge_encoder != None:
            if isinstance(self.edge_encoder, WordEncoder):
                data.edge_attr = self.edge_encoder(
                    data.edge_attr, getattr(data, "extra_edge_attr", None)
                )
            else:  # assumes MLP encoder
