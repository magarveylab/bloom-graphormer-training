...
def train(
    checkpoint_dir: str = f"{experiment_dir}/bloom-rxn-ec/checkpoints",
    mlm_checkpoint_fp: str = f"{experiment_dir}/bloom-rxn-mlm/checkpoints/last.pt",
    checkpoint_name: str = "bloom-rxn-ec-{epoch:02d}-{val_loss:.2f}",
    logger_entity: str = "magarvey",
    logger_name: str = "bloom-rxn-ec",
    ...
    # model
    model = get_model(
        dm=dm, embedding_dim=embedding_dim, mlm_checkpoint_fp=mlm_checkpoint_fp
    )
    # trainer
    trainer = get_trainer(
...
parser.add_argument(
    ...
    default=f"{experiment_dir}/bloom-rxn-ec/checkpoints",
)
parser.add_argument(
    "-mlm_checkpoint_fp",
    help="Pretrained checkpoint for mlm",
    default=f"{experiment_dir}/bloom-rxn-mlm/checkpoints/last.pt",
)
parser.add_argument(
    "-checkpoint_name",
